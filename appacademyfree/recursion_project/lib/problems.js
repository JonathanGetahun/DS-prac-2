...
function reverseString(str) {
    ...
    return str.charAt(lastChar) + reverseString(str.slice(0,lastChar))
...
function flatten(data) {
    // con = data.reduce((acc,val) => {
    //     if (Array.isArray(val)){
    //      return acc.concat(pow(val))
    //     }
    //     return acc.concat(val)
    //  }, [])
    // return con

    //the above was my solution it wasn't that good because if I was given a string , I 
    //couldn't make turn it into a 1-D array without messing up the other tests.

    if(!Array.isArray(data)) return [data] //if given string

    let allElements = [];
    //if given array
    data.forEach((el) => {
        let flattened = flatten(el) //forEach peels each array into its deeper layer, so if you keep calling it (aka using recursion you will eventually get data = string
        allElements.push(...flattened) //(...) the spread operator, takes the inside values of the outer array and puts them into another array as single values.
    })
    return allElements;
...
function fileFinder(directories, targetFile) {
    for(key in directories) {//used for objects not arrays
        //you can write recursion assuming things, write stuff with the belief of what the recursion call should give
